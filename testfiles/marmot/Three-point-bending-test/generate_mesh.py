...
def build_rilem_tpb():
    ...
    cubit.cmd(f"nodeset 1 add node with x_coord > {x_supp_left - tol} and x_coord < {x_supp_left + tol} and y_coord < {tol}")
    cubit.cmd("nodeset 1 name 'left_support'")
    
    cubit.cmd(f"nodeset 2 add node with x_coord > {x_supp_right - tol} and x_coord < {x_supp_right + tol} and y_coord < {tol}")
    cubit.cmd("nodeset 2 name 'right_support'")
    
    # Loading Point (Top center line along the Z-axis)
    cubit.cmd(f"nodeset 3 add node with x_coord > {x_center - 5 - tol} and x_coord < {x_center + 5 + tol} and y_coord > {D - tol}")
