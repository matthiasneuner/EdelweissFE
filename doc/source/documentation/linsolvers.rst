...
.. list-table:: ``blockamg`` configuration keys
    ...
    * - ``hierarchyDropLumping``
      - ``false``
      - When ``hierarchyDropTol`` drops an entry, add it onto its row's diagonal instead of discarding it, so row sums are preserved exactly (``A_filtered @ 1 == A @ 1``) and the constant near-null-space vector survives filtering. This is the AMG literature's preferred filtered-matrix construction, and it verifiably does what it claims -- on a meshfree operator the relative row-sum error falls from 4.65e-02 to 3.11e-15 at ``hierarchyDropTol=1e-2``. **It was nevertheless measured to be neutral at best**: no change at ``1e-4`` and 8.1% *slower* at ``1e-2``, with outer GMRES time unchanged in both cases -- so convergence is identical and the whole penalty is the extra pass over the matrix, +32% on the hierarchy build. Off by default; plain truncation is both simpler and faster on the operators measured so far. Note also that lumping shrinks the diagonal when off-diagonals are negative (20% at ``1e-2`` on an elasticity operator), which is worth checking if you enable it on a new problem.
