        ...
        try:
            for timeStep in step.getTimeStep():
                ...
                if modelHasChanged or connectivityHasChanged or self.theDofManager is None:
                    ...
                    # Hand every linear solver the live model and DOF manager -- the one interface
                    # point a solver needs to derive whatever it wants beyond the plain (A, b) call:
                    # field layout (the base class's own default already does this, for any
                    # field-split solver), node coordinates (e.g. blockamg's rigid-body near
                    # null-space), element topology (e.g. blockamg's P1 map for p-multigrid), or
                    # anything a future solver needs that nothing here has to know about in advance.
                    # Unconditional -- LinearSolver.setModel's default derives and stores the
                    ...
                    # Optional dump of the corner/midside topology of every vector field (the
